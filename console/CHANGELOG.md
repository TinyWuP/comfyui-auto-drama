...
## 2026-09-16 · v0.13.33 — 修复"提交后任务列表空白"，重复守卫查全实例，新增总览进度条

### 本次更新内容

- **现象**：页面点提交成功（后台任务在跑），但任务区显示"还没有任务"，刷新无效
- **根因**：v0.13.32 的 GPU 选择器默认值是 `auto`，`refreshStatus` 把
  `server=auto` 原样传给 `/api/status`，`get_status` 按"归属==auto"过滤任务
  → 全部滤掉返回空。**提交**语境 auto=自动分配，**查询**语境 auto 未定义
- **修复（后端）**：`/api/status?server=auto` 视同 `all`（多实例合并总览）
- **修复（后端）**：重复提交守卫原来只查目标实例的队列——两张卡各跑一个链头
  时互相看不见，连点提交会创建并行重复链。改为遍历**全部注册实例**队列合并
  active_pids（任一实例在跑同段旧版本即拦截 F-DUP-SUBMIT）
- **新增（前端）**：任务卡列表上方**总览条**，随 8 秒轮询刷新：
  - 本项目：各状态计数（生成中/排队/等待链/完成/失败）+ 完成进度条
    （选择器停"自动/总览"时统计合并后的全部任务）
  - ComfyUI：每实例一行「GPU0 :8188 余14G · ▶ 最后两分钟_02_v4」+ 排队数，
    离线标红；新增 `/api/comfyui_progress` 提供各实例在跑任务与步进度
    （ComfyUI `/prompt_progress` 不支持时 pct 为 null，只显示任务名不显示%）
  - 空列表/服务器异常路径也渲染总览条（finally），不再一起消失

### 影响

- "自动分配"模式下提交后立即能看到全部任务（跨实例），不再空白
- 连点"提交任务"不会再多卡并行重复开链；但同段已跑完/失败后再提交仍按
  旧规则创建新版本（_vN），这是设计行为

### 验证方式

- py_compile + node --check；部署后 curl：`server=auto` 返回 20 条合并任务
  （_01 running :8189、_01_v2 running :8188、_02_v4 queued、_03.._10_v4
  waiting）；`/api/comfyui_progress` 双实例在跑任务与余量正确

---

## 2026-09-15 · v0.13.32 — 双 GPU 实例：多卡并行出片 + 显卡资源选择器

### 本次更新内容

- **背景**：部署机两张 RTX 4090（24G×2），此前单 ComfyUI 实例只用卡 0，卡 1
  常年空转；且单卡贴顶运行（峰值 22342/24000MiB）导致「最后两分钟」链
  连续多段 OOM。ComfyUI `--multi-gpu` 是模型跨卡切分，对单任务无提速，
  正确方案是**每卡一个独立实例**（第二实例 `start2.sh`：
  `CUDA_VISIBLE_DEVICES=1 --port 8189`，`start.sh` 同步绑 `CUDA_VISIBLE_DEVICES=0`）
- **任务归属**：提交/链式推进/链头自愈/降级提交时，任务记录写入
  `t["server"]`；重生成默认沿用原任务实例
- **路由**：`get_status(server, project)` 只处理归属该实例的任务；等待任务
  沿 `chain_prev` 上溯取第一个有 server 的前驱（同链同实例，确定性归属，
  daemon 多分组不会重复提交同一任务）；`/api/status?server=all` 合并全部
  实例任务总览
- **守护**：`chain_daemon` 重写为**按 server 分组推进**——每轮把活动任务按
  归属实例分组，逐组 `get_status`（内部 advance_chain 只推进本组），单实例
  故障不阻塞其他组的链；启动时一次性迁移：旧任务按 `/history` 的真实落点
  （`ComfyUI_server` 端口）回填 server 字段，查不到归默认实例
- **跨实例续链**：失败跳过找参考帧改用全量任务搜索——前驱成功段可能在
  另一实例，末帧视频在本地共享产物目录，抽帧上传到本组实例即可继续
- **API**：新增 `/api/servers`（注册实例 + 实时负载：运行/排队数、空闲显存，
  8 秒缓存）；配置新增 `comfyui.servers` 列表（url/name/gpu），
  旧单 `server` 字段自动置顶兼容
- **前端**：第 7 步提交栏新增 **GPU 资源下拉**：`自动分配`（选运行+排队最少、
  空闲显存最大的实例，后端 `_pick_server`）/ 指定实例（显示余量与负载）/
  `全部实例总览`（状态列表跨实例合并）；任务卡多实例时显示来源端口徽章；
  选择器切换即时刷新状态；`↻` 手动刷新负载
- **诊断兜底**：all 总览分支异常捕获回显 traceback；开发中修的笔误——
  `_rank(x)` 收到 lambda 传入的状态字符串后又 `x.get("status")` → 所有带
  真实任务的总览请求 500

### 影响

- 两条（或多条）链可同时在两张卡上跑，总吞吐 ≈ ×2；单卡 OOM/重启不再
  拖垮全部生成
- 提交前可选择算力落点；"自动"模式无需关心
- 兼容：单实例部署（无 servers 配置）行为与旧版一致；旧任务无 server
  字段自动归默认实例

### 验证方式

- `py_compile` ×2、node --check 前端内联脚本、归属路由四断言单测
  （同链上溯/跨实例/旧任务默认）全过
- 服务器实测：`/api/servers` 两实例 ok、各见 24110MB（绑卡生效，
  修复前默认实例误见 48GB）；「最后两分钟」`_06.._10` 迁到卡 1 后
  `[daemon:8189]` 分组推进，`_08` 完成自动续提 `_09`（4da991ac）；
  `server=all` 总览 9 条任务带来源端口、排序正确
...
- **现象**：新建项目「最后两分钟」后，第 7 步下方"合成历史"仍显示上一个项目
  （麦田告别）的合成视频，与 v0.13.29 修的任务列表是同一类问题
- **根因**：`/api/assemble_history` 扫的是共享素材目录下**全部** `合成_*.mp4`，
  不按项目隔离；合成产物文件名规律为 `合成_<项目slug>[_段范围]_<时间戳>.mp4`，
  项目信息只在文件名里
- **修复（后端）**：`/api/assemble_history` 新增 `project` 查询参数，按
  `合成_<slug(project)>_` 前缀过滤（与 `_slug` 输出同规则，中文项目名直接保留）
- **修复（前端）**：`loadAssembleHistory` 请求带上 `?project=<当前项目名>`；
...
- 切换项目后"合成历史"只显示本项目合成的视频；未合成过的新项目显示空提示
- 不带 project 参数调用仍返回全量（兼容旧行为/运维排查）

### 验证方式

- `py_compile` + 内联脚本语法通过；slug 前缀单测（`合成_麦田告别_1-10_*.mp4`
  命中"麦田告别"、不命中"最后两分钟"）
- 服务器部署后 curl 双项目：麦田告别 1 条、最后两分钟 0 条
- ego-browser 端到端：切项目后任务卡与合成历史同步刷新

---

## 2026-09-15 · v0.13.29 — 提交生成页任务列表按项目过滤

### 本次更新内容

- **现象**：新建项目「最后两分钟」后切到第 7 步"提交生成"，页面显示的还是上一个
  项目（麦田十年）的 19 条视频任务，新项目本应是空的
- **根因（后端全局任务池）**：`state["tasks"]` 是全系统共用的一维列表，`get_status`
  直接返回全部任务，与"当前项目"无关。项目库（projects / project）按名隔离，但提交
  记录不隔离，所以任何项目看到的都是同一份任务流水
- **修复（后端 `batch_console.py`）**：
  - `get_status(server, project="")` 新增 project 参数：指定时只保留命名前缀匹配的
    任务（含 `_vN` 历史版本）
...
- 切换项目后第 7 步只显示该项目自己提交的任务；新项目未提交 → 显示"还没有任务"空态
- 链式守护 `chain_daemon` / 重新生成等内部调用不传 project，仍按全量池处理，逻辑不变
- 若某项目任务命名与项目名/剧本标题都对不上（历史手工导入等极端情况）会不在列表显示，
  但任务本身仍在跑、不影响出片与链式推进

### 验证方式

- `py_compile` + 内联脚本 `new Function` 语法通过
- 单测前缀匹配：麦田项目命中全部 21 条（v6/v7/无后缀），「最后两分钟」命中 0 条
- 服务器部署后浏览器实测：切到"最后两分钟"→ 提交生成页空态；切回"麦田告别"→ 仍显示 10 段

---

## 2026-09-15 · v0.13.28 — 修复 AI 扩写过程中分段卡片不实时加载

### 本次更新内容

- **现象**：第 4 步"生成 H3 规范提示词"勾选 AI 扩写后，后台明明每段完成就
  `_save_expand_progress` 写库，但页面分段卡片要等**全部扩写结束**才一次性更新，
  中间看不到"扩写一段加载一段"
- **根因（pollExpandTask 刷新条件写错）**：轮询里判断是否重拉用
  `saved.length !== promptTasks.length`——扩写全程任务数恒定（如 10 段），
  该条件永远为假，实时回填逻辑从未触发（后端进度条数字在动，卡片内容不动）
- **新增 `pullExpandedTasks()`** 替换该判断：
  - 卡片未渲染或段数变化 → 整表 `renderPromptCards()` 一次
  - 否则**逐段 diff**：后端该段 prompt 与当前缓存不同才回填（同时更新该段
    textarea 内容，不整表重渲染，滚动位置/其它段编辑状态都保留）
  - 保护：该段 textarea 正聚焦、或 DOM 内容与缓存不一致（用户有未提交手改）
    → 跳过回填不覆盖用户输入
- **纯前端改动**：后端 `_save_expand_progress` 链路本来就正确，无需重启服务

### 影响

- AI 扩写期间每完成一段，对应卡片约 3 秒内（轮询间隔）自动加载扩写结果
- 断线重连 / resumeExpand 恢复轮询同样受益（同一函数）

### 验证方式

- `node --check` 内联脚本语法通过
- 浏览器实测：构造扩写任务，确认 done=1 时第 1 段卡片自动更新、
  done=2 时第 2 段更新，已手改段不被覆盖
...
- **背景**：重提交按 `_vN` 版本化保留历史任务（P2 迭代对比用），同一段多版本混排在
  状态列表里，19 条任务里实际只有 10 个片段，看着乱
- **提交生成页工具栏**（刷新按钮右侧）新增复选框"仅显示每段最新版本"：
  - 按任务名去掉 `_vN` 后缀归组为"段"，每段只显示版本号最大的任务；无版本后缀视为 v1
  - 安全兜底：非最新版的任务若仍处于 running/queued/waiting（在跑），始终保留显示，
    不会被过滤掉导致看不到进度
  - 列表上方独立一行提示"已隐藏 N 个旧版本（共 X 条任务 / Y 段）"（不分页时也显示）
- **纯前端改动**：过滤在 `renderStatusCards` 内做（`visibleTasks()`），后端
  `/api/status` 与链式推进逻辑不受影响；勾选状态存 localStorage，刷新页面保留
...
- `node --check` 内联脚本语法通过
- 用服务器真实 19 条状态数据跑过滤：19 → 10，每段保留最新版、无版本段正确归组
- 部署后浏览器实测：勾选后列表只剩每段最新版，翻页计数与"已隐藏 N 个旧版本"提示正确
...
- **问题**：服务器日志反复出现 `[chain] 抽帧失败：麦田十年_01`，原 `extract_last_frame`
  只跑一次 `ffmpeg -sseof -0.1` 且静默返回 None，无任何原因信息，无法定位
  （ffmpeg 缺失 / mp4 seek 失败 / 下载不完整都表现为同一句）
- **两档抽帧**：先 `-sseof -0.1` 尾部反向 seek（快）；失败再用
  `select=gte(t,dur-0.2)` 全片解码取末帧（慢但稳，兼容 moov 尾置/时间戳异常导致 seek 失败）
- **失败诊断**：打印 ffmpeg stderr 最后一行（含文件名），区分"服务器没有 ffmpeg"
  （FileNotFoundError 单独提示安装）与解码/seek 错误；同一文件 60 秒限流一次，避免轮询刷屏
- 复制视频异常（文件损坏/不存在）单独提示并带路径

### 影响与验证

- 仅影响链式衔接抽帧路径，正常视频行为不变；本机验证：正常 mp4 抽帧成功（18KB PNG）、
  截断 mp4 输出诊断行 `Error opening input files: Invalid data found when processing input`
- `python3 -m py_compile` 通过；服务器部署后若仍失败，日志中的诊断行即为根因
...
- **前端 `genAllAssets`**：单张失败不再 throw 中断整批（旧逻辑第 2 张出错整批停止，
  表现为"提示 44 张只出了 1 张"）。改为逐张 try/catch 继续跑完，进度条统计"已处理"，
  结束后汇总 toast 失败清单（前 3 条+总数），完整清单进 console.warn；缺失项再点一次即续跑
- **后端 `_img_openai`**：429/5xx 指数退避重试（0/4s/12s/30s 共 4 次机会）——批量连发
  易触发百炼 QPS 限流（实测单张约 45s，44 张连续提交）；4xx 参数/鉴权类错误不重试直接失败
- 生成耗时提示修正：每张约 30-60 秒（实测百炼 qwen-image-3.0 含质检约 45s/张）

### 影响与注意事项

- 需重启控制台生效；44 张全量约 45s×44≈33 分钟（含退避会更久），建议挂机跑完再刷新
- 若部分分镜/角色图反复失败，看 toast 汇总与浏览器 console 的具体错误（内容审核类改提示词）
- 验证方式：浏览器实测部署机连续 2 张 asset_gen 均 200 成功（各约 46s）；
  模拟 429 首拒→退避后第 2 次成功、400 单次不重试两用例通过

---

## 2026-09-14 · v0.13.19 — 存储目录自动创建（修新部署机生图落盘 ENOENT 误报云端失败）

### 本次更新内容

- 启动时 `_ensure_storage_dirs()` 自动创建 config `storage.output_dir` 与
  `storage.asset_dirs` 全部目录（此前依赖人工预建，新部署机缺失时
  生图成功后写盘 ENOENT，被回退链误报成「云端生图失败」）
- 新增 `_save_asset()`：素材/图片统一落盘入口，写盘前 `makedirs(exist_ok=True)`，
  接入 4 处写盘点（OpenAI 云生图、DashScope 云生图、本地 Boogu、前端 base64 上传）；
  合成视频、合并视频两处 ffmpeg 输出前同样补建目录

### 影响与注意事项

- 需重启控制台生效；重启瞬间即会建好缺失目录，历史图片不受影响
- 排障提示：`云端生图失败：[Errno 2] No such file or directory` 属存盘问题而非云端问题，
  真实云端调用已成功；本版本起不再出现该误报
- 验证方式：模拟新部署机（config 指向不存在的 素材/出镜素材/outputs），
  import 后三目录自动创建；运行期删除 素材/ 后 `_save_asset` 自动重建并落盘成功

---

## 2026-09-14 · v0.13.18 — 云端生图错误透出（修百炼 400 被本地拒连假象掩盖）

### 本次更新内容

- `_img_openai` / `_boogu_local` 捕获上游 HTTPError 时读取错误体，抛
  `云端生图 HTTP 400：{"error":{...}}` 形式的真实错误（百炼返回的 code/message 直接可见）
- `boogu_generate` 回退链修正：provider=cloud 时云端失败→本地回退，若本地也不可用，
  最终异常改为「云端生图失败：<云端真实错误>（本地回退也失败：<本地错误>）」——
  此前只抛最后的本地 `Connection refused`，真实根因（如百炼 400 model_not_found/额度/地域）被掩盖
- 已按百炼 qwen-image 3.0 OpenAI 兼容文档核对：请求体平铺顶层、size 用 `宽x高`、
  响应 `data[0].url` 下载（response_format=b64_json 被其忽略，代码本就兼容 url 分支）

### 影响与注意事项

- 需重启控制台生效；接口仍返回 400，但 error 文案变成可诊断的真实原因
- 验证方式：模拟云端 400（model_not_found）+ 本地拒连，错误链完整透出；
  模拟百炼 url 响应，图片下载落盘正确；两用例通过
...
- `batch_console.py` 的 `main()` 不再硬编码绑定 `127.0.0.1`，改为默认监听 `0.0.0.0`，
  同一局域网内其他设备可通过 `http://<本机IP>:8890` 访问控制台
- 新增配置项 `config.json → console.host`（默认 `0.0.0.0`），可改回 `127.0.0.1` 仅本机访问；
  环境变量 `BATCH_CONSOLE_HOST` 优先级更高
- 端口缺省值改为读取 `console.port` 配置（命令行参数仍可覆盖）
- 启动日志同时打印监听地址；`config.example.json`、`CONFIG.md` 同步补充 `host` 字段说明

### 影响与注意事项

- **安全**：控制台无鉴权，监听 `0.0.0.0` 后局域网内任何设备都能操作（提交任务、删文件接口等），
  请在可信网络使用；不需要远程访问时在 config.json 设 `"console": {"host": "127.0.0.1"}` 关闭
- macOS 首次启动可能在「系统设置 → 网络」弹窗询问防火墙许可，需允许入站连接
- 验证方式：`python3 batch_console.py 8891` 后 `lsof -nP -iTCP:8891 -sTCP:LISTEN` 显示 `*:8891 (LISTEN)`；
  已实测通过
- 需重启服务生效（`python3 start_daemons.py` 会自动杀旧实例）
